...
| --- | --- | --- |
| a footnote storage with a kind, a stylesheet and a string | 2921 of 2921 storages carry `table_para_style`, `table_para_data`, `table_list_style`, `in_document`, `table_para_starts`, `table_para_bidi` | no paragraph style — the omission that rendered a body unstyled once already |
...
in all 26 IWA-format Pages fixtures (the corpus's one remaining `.pages` file is
...
fixtures here, 2921 storages carry the field and **zero** declare it. What
...
| --- | --- | ---: | ---: |
| `TSWP.StorageArchive` | `style_sheet` | 2921 | **0** |
