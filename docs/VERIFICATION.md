...
| 5 | 🟠 medium | Drawables & media → Image cropping (set, move, remove a mask) | a mask this library writes crops the way Apple's does | manual |
...
| 8 | 🟠 medium | Numbers & tables → Chart appearance: axes, legend, gridlines | Pages draws the chart without the gridlines this library switched off. | manual |
| 9 | 🟠 medium | Numbers & tables → Chart data editing (values, names, series, categories) | a series added or removed here leaves the chart's styling on the right series | manual |
| 10 | 🟠 medium | Numbers & tables → Conditional formatting: apply an existing rule set to more cells | re-pointing a cell's conditional-style key makes Numbers apply that rule set to it | manual |
| 11 | 🟠 medium | Numbers & tables → Filters: enable, disable, combining mode | enabling a filter set makes Numbers apply its rules | manual |
| 12 | 🟠 medium | Numbers & tables → Formula function names | The function-index table is incomplete, and every unnamed id is visible rather than guessed. | `test:e2e` |
| 13 | 🟠 medium | Numbers & tables → Formula reading (AST rendered to text) | Rendered formula text matches what the app shows in its formula bar. | manual |
| 14 | 🟠 medium | Numbers & tables → Table structure (rows, columns, bands, sizes, freeze, repeat) | Changed band counts, freeze and repeating-header flags, row heights and column widths take effect. | manual |
| 15 | 🟠 medium | Numbers & tables → Table styling (banded rows, grid strokes, visibility) | Banded rows, grid strokes and the visibility toggles render as set. | manual |
...
### 4. Floating (non-inline) drawable placement
...
### 5. Image cropping (set, move, remove a mask)
...
### 8. Chart appearance: axes, legend, gridlines
...
### 9. Chart data editing (values, names, series, categories)
...
### 10. Conditional formatting: apply an existing rule set to more cells
...
### 11. Filters: enable, disable, combining mode
...
### 13. Formula reading (AST rendered to text)
...
### 14. Table structure (rows, columns, bands, sizes, freeze, repeat)
...
### 15. Table styling (banded rows, grid strokes, visibility)
...
### 16. Drawable shadows (enabled, angle, offset, blur, opacity)
...
### 17. Categories: enable or disable grouping
...
### 18. Conditional formatting rules
