...
export const CAPABILITIES: Capability[] = [
  ...
  },
  {
    ...
    manualProof: {
      ...
      why:
        "The first in-app check crashed Pages on open. Cause, measured: every newly created " +
        "attribute table was seeded with an objectless entry at index 0 — harmless in " +
        "run-shaped tables, where it means \"nothing in effect\", fatal in the point-anchored " +
        "`table_footnote`/`table_attachment`, where an entry is an object at a position and " +
        "the numbering walk dereferences it. 107 point-anchored tables in the corpus carry " +
        "zero objectless entries. The seed is now shape-aware, and the earlier shape-audit " +
        "fix (the six attribute tables all 2676 corpus storages carry) still stands.",
      how:
